...
SwRedisClient::SwRedisClient(const std::string& uri) {
  try {
    ...
    obs::error("Failed to initialize Redis client", {
                                                        {"error", e.what()}
    });
    throw;
  }
