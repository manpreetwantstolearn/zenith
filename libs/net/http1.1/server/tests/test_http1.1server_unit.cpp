...
void test_large_body(int port) {
  ...
  // Note: Our simple server implementation might not read the full body in the handler if we don't
  // parse it, but it should accept the request.
  auto res = send_request(port, "POST", "/echo", large_body);
  // We expect 200 OK because we have a handler (even if it doesn't echo back fully in this simple
  // test setup)
  assert(res.find("200 OK") != std::string::npos);
