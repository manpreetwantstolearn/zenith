...
class Session : public std::enable_shared_from_this<Session> {
  ...
  Server::Handler m_handler; // Copy of handler (std::function is cheap to copy usually, or use
                             // reference if lifetime guaranteed)
...
private:
  ...
  void process_request() {
    ...
    // Let's look at Http1Request constructor: explicit Request(boost::beast::http::request<...>
    // req); It takes by value (copy or move).
    ...
    auto send_lambda = [self](http::response<http::string_body> msg) {
      ...
      http::async_write(self->socket_, *sp, [self, sp](beast::error_code ec, std::size_t bytes) {
...
void Server::do_accept() {
  m_acceptor.async_accept(
      net::make_strand(m_ioc), [this](beast::error_code ec, tcp::socket socket) {
        if (!ec) {
