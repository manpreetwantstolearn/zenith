...
ClientDispatcher::ClientDispatcher(const ClientConfig& config) : m_config(config) {
  m_work =
...
void ClientDispatcher::submit(const std::string& host, uint16_t port, const std::string& method,
...
NgHttp2Client* ClientDispatcher::get_or_create(const std::string& host, uint16_t port) {
