...
ResponseHandle::ResponseHandle(SendFunction send_fn, boost::asio::io_context& io_ctx) :
    m_send_fn(std::move(send_fn)), m_io_ctx(io_ctx), m_stream_alive(true) {
}

void ResponseHandle::send(int status, std::map<std::string, std::string> headers,
                          std::string body) {
  ...
  boost::asio::post(m_io_ctx,
                    [self, status, headers = std::move(headers), body = std::move(body)]() mutable {
                      // On io_context thread now - safe to access response
                      if (self->m_stream_alive.load(std::memory_order_acquire)) {
                        self->m_send_fn(status, std::move(headers), std::move(body));
                      }
