...
void ResponseHandle::send(int status, std::map<std::string, std::string> headers, std::string body) {
    // Post to io_context where response object lives
    // Capture shared_ptr to keep handle alive until lambda executes
    auto self = shared_from_this();
    
    boost::asio::post(m_io_ctx, [self, status, headers = std::move(headers), body = std::move(body)]() mutable {
