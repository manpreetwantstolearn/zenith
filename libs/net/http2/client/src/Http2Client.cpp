...
void ClientImpl::connect() {
  try {
    ...
    m_session = std::make_unique<nghttp2::asio_http2::client::session>(m_io_context, m_config.host,
                                                                       m_config.port);
...
void ClientImpl::schedule_retry(const std::string& method, const std::string& path,
                                ...
                                const std::map<std::string, std::string>& headers,
                                ResponseHandler handler, int retries_remaining,
  ...
  if (!m_connected) {
    if (retries_remaining > 0) {
      ...
      timer->async_wait([this, method, path, body, headers, handler, retries_remaining, last_error,
        ...
        if (!ec) {
          ...
          schedule_retry(method, path, body, headers, handler, retries_remaining - 1,
      ...
      handler(Response(),
              Error{1, "Not connected: " + (last_error.message.empty() ? "Connection failed"
                                                                       : last_error.message)});
      return;
    }
  }
  ...
  if (ec) {
    if (retries_remaining > 0) {
      std::shared_ptr<boost::asio::deadline_timer> timer =
          std::make_shared<boost::asio::deadline_timer>(m_io_context);
      timer->expires_from_now(boost::posix_time::milliseconds(m_config.retry_interval.count()));
      timer->async_wait([this, method, path, body, headers, handler, retries_remaining, ec,
  ...
  timer->async_wait([req, state, handler, this, method, path, body, headers,
                     retries_remaining](const boost::system::error_code& ec) {
  ...
  req->on_response([this, state, timer, handler, method, path, body, headers,
                    retries_remaining](const nghttp2::asio_http2::client::response& res) {
  ...
  req->on_close([this, state, timer, handler, method, path, body, headers,
                 retries_remaining](uint32_t error_code) {
    if (state->completed) {
      return; // Already handled (e.g. timeout)
    }
    ...
    if (error_code != 0) {
      ...
      if (retries_remaining > 0) {
        schedule_retry(method, path, body, headers, handler, retries_remaining - 1,
                       Error{(int)error_code, "Stream closed with error"});
      } else {
      ...
      if (m_config.retry_on_server_error && status >= 500 && status < 600 &&
          retries_remaining > 0) {
        schedule_retry(method, path, body, headers, handler, retries_remaining - 1,
