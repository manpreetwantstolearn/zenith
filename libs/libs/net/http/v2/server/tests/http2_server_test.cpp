...
TEST(Http2ServerTest, Construction) {
  auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9001));
  EXPECT_NE(server, nullptr);
}

TEST(Http2ServerTest, ConstructionWithDifferentPorts) {
  auto server1 = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9101));
  auto server2 = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9102));
  auto server3 = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9103));
...
TEST(Http2ServerTest, BindToAllInterfaces) {
  auto server = std::make_unique<zenith::http2::Server>(make_config("0.0.0.0", 9007));
  EXPECT_NE(server, nullptr);
}
...
TEST(Http2ServerTest, HandlerRegistration) {
  auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9002));
...
TEST(Http2ServerTest, MultipleHandlers) {
  auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9003));
...
TEST(Http2ServerTest, SamePathDifferentMethods) {
  auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9010));
...
TEST(Http2ServerTest, HandlerWithPathParams) {
  auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9011));
...
TEST(Http2ServerTest, ManyHandlersStress) {
  auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9012));
...
TEST(Http2ServerTest, ThreadConfiguration) {
  EXPECT_NO_THROW({
    auto server1 = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9004, 1));
    auto server2 = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9005, 2));
    auto server4 = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9006, 4));
  });
}

TEST(Http2ServerTest, ManyThreadsConfiguration) {
  EXPECT_NO_THROW({
    auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9013, 16));
  });
}
...
TEST(Http2ServerTest, StressConstruction) {
  for (int i = 0; i < 100; ++i) {
    auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9008));
    EXPECT_NE(server, nullptr);
  }
}
...
TEST(Http2ServerTest, ConcurrentConstruction) {
  ...
  for (int i = 0; i < 10; ++i) {
    threads.emplace_back([i, &success_count]() {
      try {
        auto server = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9100 + i));
        if (server) {
          success_count++;
        }
      } catch (...) {
...
protected:
  void SetUp() override {
    server_ = std::make_unique<zenith::http2::Server>(make_config("127.0.0.1", 9009));
  }
  ...
  std::unique_ptr<zenith::http2::Server> server_;
