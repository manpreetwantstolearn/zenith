...
bool JsonDocument::contains(const std::string& key) const {
    if (!m_impl->val.is_object()) return false;
    return m_impl->val.as_object().contains(key);
}

std::string JsonDocument::get_string(const std::string& key) const {
    if (!contains(key)) throw std::runtime_error("Key not found: " + key);
    const auto& v = m_impl->val.as_object().at(key);
    if (!v.is_string()) throw std::runtime_error("Value is not a string: " + key);
...
int JsonDocument::get_int(const std::string& key) const {
    if (!contains(key)) throw std::runtime_error("Key not found: " + key);
    const auto& v = m_impl->val.as_object().at(key);
    if (!v.is_int64()) throw std::runtime_error("Value is not an integer: " + key);
...
bool JsonDocument::get_bool(const std::string& key) const {
    if (!contains(key)) throw std::runtime_error("Key not found: " + key);
    const auto& v = m_impl->val.as_object().at(key);
    if (!v.is_bool()) throw std::runtime_error("Value is not a boolean: " + key);
    return v.as_bool();
}
...
bool JsonDocument::is_object() const { return m_impl->val.is_object(); }
bool JsonDocument::is_array() const { return m_impl->val.is_array(); }
bool JsonDocument::is_string() const { return m_impl->val.is_string(); }
bool JsonDocument::is_number() const { return m_impl->val.is_number(); }
