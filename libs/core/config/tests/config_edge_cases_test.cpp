...
TEST_F(ConfigProviderEdgeCasesTest, RapidSequentialReloads) {
  ...
  auto provider =
      ConfigProvider::create(std::move(source_ptr), std::move(parser), nullptr, metrics);
...
TEST_F(ConfigProviderEdgeCasesTest, CallbackThrowsException) {
  ...
  EXPECT_TRUE(std::any_of(errors.begin(), errors.end(), [](const std::string& err) {
    return err.find("Callback failed") != std::string::npos;
  }));
}
...
TEST_F(ConfigProviderEdgeCasesTest, InvalidVersionNumber) {
  ...
  auto errors = logger->getErrors();
  EXPECT_TRUE(std::any_of(errors.begin(), errors.end(), [](const std::string& err) {
    return err.find("version") != std::string::npos;
  }));
}
