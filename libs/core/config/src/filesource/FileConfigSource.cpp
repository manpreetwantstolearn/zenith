...
FileConfigSource::FileConfigSource(std::filesystem::path config_file_path,
                                   std::shared_ptr<zenith::execution::IExecutor> executor) :
    m_config_file_path(std::move(config_file_path)), m_executor(std::move(executor)),
    m_inotify_fd(-1), m_watch_fd(-1), m_timer_fd(-1) {
  if (!m_executor) {
...
std::string FileConfigSource::fetchConfig() {
  ...
  obs::debug("FileConfigSource: Read " + std::to_string(buffer.str().size()) + " bytes from " +
...
void FileConfigSource::start() {
  ...
  if (m_inotify_fd == -1) {
    obs::error("FileConfigSource: Failed to initialize inotify: " +
  ...
  if (m_watch_fd == -1) {
    obs::error("FileConfigSource: Failed to add inotify watch: " +
               std::string(std::strerror(errno)));
    close(m_inotify_fd);
    m_inotify_fd = -1;
    return;
  ...
  if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, m_inotify_fd, &ev) == -1) {
    obs::error("FileConfigSource: Failed to add inotify fd to epoll: " +
               std::string(std::strerror(errno)));
    close(m_timer_fd);
    close(m_stop_event_fd);
    close(m_epoll_fd);
  ...
  if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, m_stop_event_fd, &ev) == -1) {
    obs::error("FileConfigSource: Failed to add event fd to epoll: " +
               std::string(std::strerror(errno)));
    close(m_timer_fd);
    close(m_stop_event_fd);
    close(m_epoll_fd);
  ...
  if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, m_timer_fd, &ev) == -1) {
    obs::error("FileConfigSource: Failed to add timer fd to epoll: " +
               std::string(std::strerror(errno)));
    close(m_timer_fd);
    close(m_stop_event_fd);
    close(m_epoll_fd);
...
void FileConfigSource::stop() {
  ...
  m_stop_cv.wait(lock, [this] {
    return !m_task_active.load();
  });
  ...
  while (m_running.load()) {
    ...
    for (int n = 0; n < nfds; ++n) {
      ...
      if (events[n].data.fd == m_timer_fd) {
        ...
        if (m_callback) {
          obs::info("FileConfigSource: File modification detected (debounced): " +
                    m_config_file_path.string());
          try {
        ...
        if (length < 0) {
          if (errno != EAGAIN) {
            obs::error("FileConfigSource: inotify read error: " +
                       std::string(std::strerror(errno)));
          }
          continue;
        }
        ...
        if (file_changed) {
          ...
          if (timerfd_settime(m_timer_fd, 0, &ts, nullptr) == -1) {
            obs::error("FileConfigSource: Failed to arm timer: " +
                       std::string(std::strerror(errno)));
          }
        }
      }
