...
zenith::Result<ConfigProvider, std::string> ConfigProvider::create(
    std::unique_ptr<IConfigSource> source, std::unique_ptr<IConfigParser> parser,
    std::shared_ptr<IConfigLogger> logger, std::shared_ptr<IConfigMetrics> metrics) {
  if (!logger) {
  ...
  try {
    ...
    return zenith::Result<ConfigProvider, std::string>::Ok(
        ConfigProvider(std::move(source), std::move(parser), logger, metrics, config_ptr));
...
ConfigProvider::ConfigProvider(ConfigProvider&& other) noexcept :
    m_source(std::move(other.m_source)), m_parser(std::move(other.m_parser)),
    m_logger(std::move(other.m_logger)), m_metrics(std::move(other.m_metrics)),
    m_config(std::move(other.m_config)), m_callbacks(std::move(other.m_callbacks)),
...
ConfigProvider::ConfigProvider(std::unique_ptr<IConfigSource> source,
    ...
    m_source(std::move(source)), m_parser(std::move(parser)), m_logger(std::move(logger)),
    m_metrics(std::move(metrics)), m_config(std::move(initial_config)) {
  m_source->watchForChanges([this](const std::string& new_config) {
    handleConfigChange(new_config);
  });
}
...
void ConfigProvider::notifyCallbacks(const Config& new_config) {
  ...
  m_logger->info("ConfigProvider: Notifying " + std::to_string(callbacks_copy.size()) +
                 " callback(s)");
