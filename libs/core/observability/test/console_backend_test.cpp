...
TEST_F(ConsoleBackendTest, LogOutput) {
  ...
  std::string output = getCapturedOutput();
  EXPECT_NE(output.find("[INFO] Test info message"), std::string::npos) << "Output was: " << output;
  EXPECT_NE(output.find("[WARN] Test warning"), std::string::npos) << "Output was: " << output;
  EXPECT_NE(output.find("[ERROR] Test error"), std::string::npos) << "Output was: " << output;
}
...
TEST_F(ConsoleBackendTest, CounterOutput) {
  ...
  std::string output = getCapturedOutput();
  EXPECT_NE(output.find("[COUNTER] requests += 1"), std::string::npos) << "Output was: " << output;
  EXPECT_NE(output.find("[COUNTER] requests += 5"), std::string::npos) << "Output was: " << output;
}
...
TEST_F(ConsoleBackendTest, HistogramOutput) {
  ...
  std::string output = getCapturedOutput();
  EXPECT_NE(output.find("[HISTOGRAM] latency"), std::string::npos) << "Output was: " << output;
}
...
TEST_F(ConsoleBackendTest, SpanIncludesTraceId) {
  ...
  std::string output = getCapturedOutput();
  EXPECT_NE(output.find("trace="), std::string::npos) << "Output was: " << output;
}
