...
public:
  ConsoleSpan(std::string_view name, const Context& ctx, std::mutex& mtx) :
      m_name(name), m_ctx(ctx), m_mutex(mtx), m_start(std::chrono::steady_clock::now()) {
    std::lock_guard<std::mutex> lock(m_mutex);
    std::cerr << "[SPAN START] " << m_name << " trace=" << m_ctx.trace_id.to_hex().substr(0, 8)
              << "\n";
  }
...
public:
  ConsoleCounter(std::string_view name, std::mutex& mtx) : m_name(name), m_mutex(mtx), m_value(0) {
  }
...
public:
  ConsoleHistogram(std::string_view name, std::mutex& mtx) : m_name(name), m_mutex(mtx) {
  }
...
public:
  ConsoleGauge(std::string_view name, std::mutex& mtx) : m_name(name), m_mutex(mtx), m_value(0) {
  }
