...
public:
  Span& set_error(std::string_view message) override {
    if (m_span) {
      m_span->SetStatus(trace_api::StatusCode::kError, std::string(message));
    }
    return *this;
  }
  ...
  Context context() const override {
    ...
    if (m_span) {
      ...
      for (int i = 0; i < 8; ++i) {
        child.span_id.value = (child.span_id.value << 8) | span_ctx.span_id().Id()[i];
      }
    }
    return child;
  }
...
OTelBackend::OTelBackend(const OTelConfig& config) : m_config(config) {
  ...
  auto trace_processor =
      std::make_unique<trace_sdk::SimpleSpanProcessor>(std::move(trace_exporter));
  ...
  auto log_processor =
      std::make_unique<logs_sdk::SimpleLogRecordProcessor>(std::move(log_exporter));
  ...
  m_meter_provider = std::shared_ptr<metrics_api::MeterProvider>(new metrics_sdk::MeterProvider());
...
void OTelBackend::log(Level level, std::string_view message, const Context& ctx) {
  ...
  switch (level) {
  case Level::TRACE:
    severity = logs_api::Severity::kTrace;
    break;
  case Level::DEBUG:
  ...
  case Level::ERROR:
    ...
    break;
  case Level::FATAL:
    severity = logs_api::Severity::kFatal;
    break;
  }
...
std::shared_ptr<Counter> OTelBackend::get_counter(std::string_view name, std::string_view desc) {
  std::lock_guard<std::mutex> lock(m_mutex);
  if (m_shutdown || !m_meter) {
    return nullptr;
  }

  auto counter = m_meter->CreateUInt64Counter(std::string(name), std::string(desc));
...
std::shared_ptr<Gauge> OTelBackend::get_gauge(std::string_view name, std::string_view desc) {
  ...
  auto updown_counter = m_meter->CreateInt64UpDownCounter(std::string(name), std::string(desc));
  return std::make_shared<OTelGauge>(std::move(updown_counter));
}

std::shared_ptr<Histogram> OTelBackend::get_histogram(std::string_view name,
  ...
  auto histogram = m_meter->CreateDoubleHistogram(std::string(name), std::string(desc));
