...
ShortLink::ShortLink(ShortCode code, OriginalUrl original, ExpirationPolicy expiration,
                     TimePoint created_at) :
    m_code(std::move(code)), m_original(std::move(original)), m_expiration(std::move(expiration)),
...
ShortLink::CreateResult ShortLink::create(ShortCode code, OriginalUrl original,
  ...
  return CreateResult::Ok(
      ShortLink(std::move(code), std::move(original), std::move(expiration), Clock::now()));
}
