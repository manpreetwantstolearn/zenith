...
protected:
  ...
  zenith::http2::ClientConfig m_config;
...
TEST_F(HttpDataServiceAdapterTest, SaveTranslatesToPost) {
  ...
  DataServiceRequest req{DataServiceOperation::SAVE,
                         ...
                         R"({"code":"abc123","url":"https://example.com"})", nullptr, nullptr};
...
TEST_F(HttpDataServiceAdapterTest, FindTranslatesToGet) {
  ...
  DataServiceRequest req{DataServiceOperation::FIND, "abc123", "", nullptr, nullptr};
...
TEST_F(HttpDataServiceAdapterTest, DeleteTranslatesToDelete) {
  ...
  DataServiceRequest req{DataServiceOperation::DELETE, "xyz789", "", nullptr, nullptr};
...
TEST_F(HttpDataServiceAdapterTest, UnknownServiceThrows) {
  ...
  EXPECT_THROW({ adapter.execute(req, [](DataServiceResponse) {}); }, std::runtime_error);
}
...
TEST_F(HttpDataServiceAdapterTest, CustomBasePathUsed) {
  ...
  DataServiceRequest req{DataServiceOperation::FIND, "test123", "", nullptr, nullptr};
...
TEST_F(HttpDataServiceAdapterTest, ConcurrentRequestsHandledCorrectly) {
  ...
  for (int i = 0; i < num_requests; i++) {
    DataServiceRequest req{DataServiceOperation::FIND, "code" + std::to_string(i), "", nullptr,
                           nullptr};
