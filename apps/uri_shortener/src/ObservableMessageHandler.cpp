...
ObservableMessageHandler::ObservableMessageHandler(zenith::execution::IMessageHandler& inner) :
    m_inner(inner), m_messages_processed(obs::counter("uri_shortener.messages.processed",
                                                      "Total messages processed")),
    m_messages_failed(obs::counter("uri_shortener.messages.failed", "Total messages failed")),
    m_processing_time(
        obs::histogram("uri_shortener.messages.duration_ms", "Message processing time in ms")) {
}
...
std::string ObservableMessageHandler::get_message_type(const UriPayload& payload) {
  return std::visit(overloaded{[](const HttpRequestMsg&) {
                                 return std::string("http_request");
                               },
                               [](const DbQueryMsg&) {
                                 return std::string("db_query");
                               },
                               [](const DbResponseMsg&) {
                                 return std::string("db_response");
                               }},
                    payload);
}
