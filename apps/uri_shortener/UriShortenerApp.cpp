...
UriShortenerApp::UriShortenerApp(std::shared_ptr<domain::ILinkRepository> repo,
    ...
    m_repo(std::move(repo)), m_gen(std::move(gen)), m_shorten(std::move(shorten)),
    m_resolve(std::move(resolve)), m_delete(std::move(del)), m_msg_handler(std::move(msg_handler)),
    m_obs_msg_handler(std::move(obs_msg_handler)), m_pool(std::move(pool)),
    m_req_handler(std::move(req_handler)), m_obs_req_handler(std::move(obs_req_handler)),
...
zenith::Result<UriShortenerApp, AppError> UriShortenerApp::create(const Config& config) {
  ...
  auto gen = config.code_generator ? config.code_generator
  ...
  auto inner_msg_handler = std::make_unique<UriShortenerMessageHandler>(shorten, resolve, del);
  ...
  auto pool =
      std::make_unique<zenith::execution::StripedMessagePool>(thread_count, *obs_msg_handler);
  ...
  return zenith::Result<UriShortenerApp, AppError>::Ok(UriShortenerApp(
      std::move(repo), std::move(gen), std::move(shorten), std::move(resolve), std::move(del),
      std::move(inner_msg_handler), std::move(obs_msg_handler), std::move(pool),
      std::move(req_handler), std::move(obs_req_handler), std::move(server)));
}
